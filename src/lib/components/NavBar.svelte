...
<header class="fixed top-0 left-0 z-50 w-full border-b border-gray-200 bg-gray-50 shadow-sm dark:border-gray-700 dark:bg-gray-900">
	<div class="flex flex-col items-center px-6 py-4">
		...
		<ul class="mt-2 flex flex-wrap justify-center gap-4 md:space-x-6">
			{#each navLinks as link}
				<li>
					<a href={link.href} class="relative transition-colors duration-200 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">
						{link.name}
						<span
			...
			{#if results.length > 0}
				...
				>
					{#each results as item}
						<li>
							<a href={item.link} class="block px-4 py-2 transition hover:bg-gray-100 dark:hover:bg-gray-700">
